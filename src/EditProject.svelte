<script>
    ...
    function getParams() {
        ...
        const project_id = pathSegments[pathSegments.length - 1];
        return { project_id };
    }
    ...
    onMount(() => {
        ...
        if (project_id) {
            ...
            console.error("project_id is null or undefined");
        }
    });
    ...
    async function addElement(event) {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:5000/edit_project/${$project._id}/add_element`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${accessToken}`
    ...
    async function addImageStandard(event) {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:5000/edit_project/${$project._id}/add_image`, {
                method: "POST",
                headers: {
                    "Authorization": `Bearer ${accessToken}`
                },
    ...
    const fetchPriceEstimate = async () => {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:5000/edit_project/${$project._id}/get-price-estimate`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${accessToken}`
            ...
            if (response.ok) {
                const data = await response.json();
                priceEstimate.set(data.price_estimate);
            } else {
    ...
    let priceEstimate = writable("");


    function showAiHelper() {
        var modal = document.getElementById("AiHelper");
        modal.style.display = "block";
    }
    ...
    async function deleteFinalNote() {
        const accessToken = localStorage.getItem('accessToken');

        try {
            const response = await fetch(`http://127.0.0.1:5000/api/deleteFinalNote/${$project._id}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${accessToken}`
    ...
    const AddFinalNote = async () => {
        const accessToken = localStorage.getItem('accessToken');
        const final_note = $priceEstimate;

        try {
            const response = await fetch(`http://127.0.0.1:5000/edit_project/${$project._id}/add-final-note`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${accessToken}`
                },
                body: JSON.stringify({ final_note })
            });
    ...
    let finalNoteInput = writable(""); // Входное значение для final_note

    let isEditing = writable(false);

    function editFinalNote() {
        finalNoteInput.set($project.final_note);
        isEditing.set(true);
    }
...
.modal-content-showcase {
    ...
    height: 80%;
    font-size: 26px;
    overflow-y: auto;  /* Add this line to handle vertical overflow */
...
.modal-content_for_ai__content button {
  padding: 1rem;
  border-radius: 2rem;
}
.modal-content_for_ai__content p {
  font-size: 1.4rem;
  color: #dddddd;
}
</style>
...
<main>
    ...
    {#if $project}
    <div id="AiHelper" class="modal">
        <div class="modal-content_for_ai">
            <span class="close" on:click={closeAiHelper}>&times;</span>
            <div class="modal-content_for_ai__content">
                <h2>Estimate Boat Price</h2>
                {#if $project.final_note}
                    <h1>Cureent Finalization project</h1>
                    {#if $isEditing}
                        <div>{@html $priceEstimate}</div>
                        <button on:click={AddFinalNote} class="modal-button">Save</button>
                        <button on:click={cancelEdit} class="modal-button">Cancel</button>
                    {:else}
                        <p>{@html $project.final_note}</p>
                        <button on:click={deleteFinalNote} class="modal-button">Delete Finalization</button>
                    {/if}
                {:else}
                    <p>There is no Finalization</p>
                    <p >{@html $priceEstimate}</p>
                    <button on:click={AddFinalNote} class="modal-button">Submit</button>
                {/if}
                <p>{@html $priceEstimate}</p>
                <button on:click={fetchPriceEstimate} class="modal-button">AI Assessment</button>
            </div>
        </div>
    </div>
    <div class="toptop">
        <Link to="/main" class="btn btn-danger">Back to Create</Link><!-- кнопка перехода на страницу создания проектов -->
    ...
    <div class="sections-buttons">
        ...
        <button class="forplus" on:click={showAiHelper}>AI Аnalysis</button>
    </div>
    ...
    <div id="vetrineModal" class="modal">
        <div class="modal-content-showcase">
            ...
            <div class="preview-content">
                ...
                <div id="final_note" name="final_note" required>{@html $project.final_note}</div>
                ...
                <div class="steps-section">
                    ...
                    <div class="add-step-form">
                        <form on:submit={addStep}>
                            ...
                            <button class="forplus" on:click={showAiHelper}>update finalization</button>
                        </form>
                    </div>
                    </div>
                </div>
